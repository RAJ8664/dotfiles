# Generated by Powerlevel10k configuration wizard on 2025-04-08 at 19:35 IST.
...
#   network_icon           #displays whether i am connected to wifi or ethernet
    #github_notifications   #displays the github notifications;	
    #cpu_usage
)
...
#my custom functions;
#use to display whether i am connected to wifi or ethernet;
function prompt_network_icon() {
  local icon_wifi=$'\f05a9'     # 
  local icon_eth=$'\uf6ff'      # 
  local icon_offline=$'\uf071'  # 

  local color_wifi=2     # green
  local color_eth=4      # blue
  local color_offline=1  # red
  ...
  if [[ "$(uname)" == "Linux" ]]; then
    if ip a | grep -q 'state UP'; then
      if iwgetid -r &>/dev/null; then
        icon="$icon_wifi"
        color=$color_wifi
      elif ip link show | grep -A1 'state UP' | grep -q 'ether'; then
        icon="$icon_eth"
        color=$color_eth
      else
        icon="$icon_offline"
  ...
  # Print the segment with the chosen color
  [[ -n $icon ]] && p10k segment -t "$icon" -f "$color" -b default
}


#Realtime cpu usage;
function prompt_cpu_usage() {
  # Get CPU idle % (works with `sysstat`)
  local idle=$(mpstat 1 1 | awk '/Average/ && $12 ~ /[0-9.]+/ { print $12 }')
  local usage=$(printf "%.0f" "$(echo "100 - $idle" | bc)")

  # Get CPU temp (if available)
  local temp=$(sensors 2>/dev/null | grep -m 1 'Package id 0:' | awk '{print $4}' | tr -d '+°C')

  # Final output (🖥️ is the CPU icon)
  local out="🖥️ ${usage}%"
  [[ -n "$temp" ]] && out+=" | 🌡️ ${temp}°C"

  p10k segment -t "$out" -f 2 -b default
}

typeset -g POWERLEVEL9K_PROMPT_CPU_USAGE_FOREGROUND=2
typeset -g POWERLEVEL9K_PROMPT_CPU_USAGE_BACKGROUND=default
